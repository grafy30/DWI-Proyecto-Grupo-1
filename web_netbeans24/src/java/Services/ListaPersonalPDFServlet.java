...
@WebServlet("/ListaPersonalPDFServlet")
public class ListaPersonalPDFServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        response.setHeader("Content-Disposition", "inline; filename=ListaPersonal.pdf");
        ...
        try {
            ...
            Paragraph titulo = new Paragraph("Lista de Personal", fontTitulo);
            ...
            tabla.setWidths(new float[]{2, 3, 3, 3, 3, 3}); // proporciones

            Font fontCabecera = new Font(Font.FontFamily.HELVETICA, 12, Font.BOLD);
            tabla.addCell(crearCeldaCabecera("Nombre", fontCabecera));
            tabla.addCell(crearCeldaCabecera("Cargo", fontCabecera));
            tabla.addCell(crearCeldaCabecera("Correo", fontCabecera));
            tabla.addCell(crearCeldaCabecera("Telefono", fontCabecera));
            tabla.addCell(crearCeldaCabecera("Descripción", fontCabecera));
            tabla.addCell(crearCeldaCabecera("Imagen", fontCabecera));

            // Datos
            PersonalBL logica = new PersonalBL();
            List<PersonalBE> personal = logica.ReadAll();

            Font fontCelda = new Font(Font.FontFamily.HELVETICA, 11);
            for (PersonalBE p : personal) {
                tabla.addCell(crearCelda(p.getNombre_personal(), fontCelda));
                tabla.addCell(crearCelda(p.getCargo_personal(), fontCelda));
                tabla.addCell(crearCelda(p.getCorreo_personal(), fontCelda));
                tabla.addCell(crearCelda(p.getTelefono_personal(), fontCelda));
